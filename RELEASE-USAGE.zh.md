# dsh-sandbox-escalation-fix 0.1.2-alpha5 使用说明

## 版本内容

- 支持 DSH Desktop `2.0.3` 隐藏宿主包清单时的严格结构校验回退。
- 支持通过 `link:`、工作区软链接或外部插件目录加载插件。
- 支持 DSH `0.1.2-alpha.5`，本插件会按每个 Session 的实际 Sandbox Mode 与 Approval Policy 投影升级字段。
...
4. 当前支持的 DSH 版本为 `0.1.0-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8`、`0.1.1-rc.1`、`0.1.1-rc.2`、`0.1.2-alpha.1`、`0.1.2-alpha.2`、`0.1.2-alpha.3`、`0.1.2-alpha.4` 和 `0.1.2-alpha.5`。

> DSH `0.1.2-alpha.5` 仍使用注册表全局静态 Schema；Session 当前模式和严格变宽仍在执行期处理，`approval=never` 仍主要依靠提示词。官方 alpha.5 npm 包的真实构建与集成测试确认本插件仍有必要，建议只在实际遇到同模式升级、空 justification 或重复重试问题后安装。
