...
function validateDshRuntime(readManifest = (name) => require(`${name}/package.json`)) {
	const versions = {};
	...
	if (unavailablePackages.length === DSH_PACKAGES.length) return {
		mode: "structural",
		unavailablePackages
	};
	if (unavailablePackages.length > 0) throw new Error(`dsh-sandbox-escalation-fix: only some DSH package manifests are readable; unavailable packages: ${unavailablePackages.join(", ")}`);
