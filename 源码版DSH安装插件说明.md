...
支持的 DSH 版本以 [README.zh.md 的支持范围](README.zh.md#支持范围) 为准（当前包含 `0.1.5-rc.1`）。
...
```powershell
pnpm dsh plugin --profile web add dsh-sandbox-escalation-fix@0.1.5-rc1-win-linux
```
...
```powershell
pnpm dsh plugin --profile web add "D:\下载目录\dsh-sandbox-escalation-fix-0.1.5-rc1-win-linux.tgz"
```
