...
The `0.1.3-alpha1` Release adds DSH `0.1.3-alpha.1` compatibility while retaining Desktop 2.0.3, linked/external plugin resolution, Git installation support, and the BOM-free `cordis.patch.yml` from `0.1.2-alpha2.1`. Earlier packages may be exposed to repeated-BOM contamination when that file is rewritten by some Windows encoding tools; users should replace them with this Release. Download and extract `dsh-sandbox-escalation-fix-0.1.3-alpha1-release.zip`; it contains the tarball, one-click install and uninstall scripts, and a concise Chinese usage guide. Earlier Releases remain available.
...
The script builds `lib`, packages the npm tarball, then creates `dsh-sandbox-escalation-fix-0.1.3-alpha1-release.zip` in `release/`. The generated directory is ignored by Git; upload only this ZIP as the GitHub Release asset.
