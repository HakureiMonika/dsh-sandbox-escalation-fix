...
- Operating systems: Windows, Linux, and macOS. The plugin itself is pure JavaScript with no platform-specific code; actual sandbox enforcement on Linux/macOS depends on the sandbox backends available to the DSH host (Linux: `bwrap` or a Landlock-enforcing kernel 5.13+; macOS: Seatbelt), probed at runtime by DSH itself. When no backend is usable, DSH refuses to run the command rather than bypassing the sandbox. The plugin's permission projection and argument normalization do not depend on any particular backend.
...
The `0.1.3-alpha1-win-linux` Release adds DSH `0.1.3-alpha.1` compatibility and official Linux/macOS support: the plugin itself is pure JavaScript, Linux/macOS install and uninstall scripts (`.sh`) are now shipped beside the Windows (`.ps1`) ones, and the full flow was verified on real Ubuntu 24.04 (Landlock backend): test suite 40/40, `.sh` install into a `web` Profile, Schema projection confirmed, and sandbox/approval behavior identical to Windows. It retains Desktop 2.0.3, linked/external plugin resolution, Git installation support, and the BOM-free `cordis.patch.yml` from `0.1.2-alpha2.1`. Earlier packages may be exposed to repeated-BOM contamination when that file is rewritten by some Windows encoding tools; users should replace them with this Release. Download and extract `dsh-sandbox-escalation-fix-0.1.3-alpha1-win-linux-release.zip`; it contains the tarball, one-click install and uninstall scripts for both Windows (`.ps1`) and Linux/macOS (`.sh`), and a concise Chinese usage guide. Earlier Releases remain available.
